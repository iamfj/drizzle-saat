# testing-with-saat (practical)

Using saat to give every test a **fresh, realistic, deterministic** database —
...
test("keyed rows are stable anchors", async () => {
  ...
  const welcome = db.query("SELECT user_id FROM todos WHERE title = 'Welcome to saat'").get();
